...
export const Expenses = ({ route }) => {
  ...
  const [despesasValide, setDespesaValide] = useState([]);
          ...
          db.transaction(tx => {
                tx.executeSql(
                      ...
                      (_, resultSet) => {
                            ...
                            console.log('Dados lidos da tabela despesaMotoristas:', despesasValide);
                      },
                      (_, error) => {
      ...
      for (const objeto of arrayDeObjetos) {
        ...
        if (snapshot.exists()) {
          ...
          if (snapshot.val().random === random) {
            ...
            // a url não existe actualiza o nó
            if (!snapshot.val().url) {
              //a despesa não tem foto associada, atualize os dados
              await set(ref(datab, caminhoPai), objeto);
              console.log('Dados atualizados com sucesso para o nó pai:', caminhoPai);
            }
            ...
            <View>
              ...
              <View style={{
                ...
                <Text style={{
                  ...
                  Valor Usado: {totalGastos === null ? "0" : totalGastos.toFixed(2)} MT</Text>
              </View>
