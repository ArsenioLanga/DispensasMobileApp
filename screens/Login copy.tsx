      ...
      const validarLogin = ()=> {
        ...
        if(newemail == null || newpassword == null){
            ...
            if(!sessionData){
                ...
                //Logar com sessao guardada localmemte
                console.log("2")
                const objetoEncontrado = sessao.find(objeto => objeto.Usuario === newemail);
                if(objetoEncontrado) {
                    // Se o objeto foi encontrado, compare a segunda string (cor) com a propriedade do objeto
                    if(objetoEncontrado.Senha === newpassword) {
                        console.log(objetoEncontrado.Senha, newpassword)
                            if(objetoEncontrado.Status === "Activo"){

                                        // Criar sessão na bd
                                db.transaction((tx) => {
                                    tx.executeSql(
                                    'INSERT INTO sessions (usuario, senha, status) VALUES (?, ?, ?);',
                                    [objetoEncontrado.Usuario, objetoEncontrado.Senha, objetoEncontrado.Status],
                                    (_, { insertId }) => {
                                        console.log('Sessão inserida com sucesso, ID:', insertId);
                                    }
                                    );
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.white }}>
            <View style={{ flex: 1, marginHorizontal: 22 }}>
                ...
                <View style={{
                    ...
                    <Text style={{ fontSize: 16, color: COLORS.black }}>Esqueceu a senha? </Text>
                    <Pressable
